...
class App {
    ...
    meetingsShow = () => {
        this.dom.querySelector('#app>#body').replaceChildren(this.meetings.dom);
        this.meetings.list();
    }

    login = async () => {
        var email = this.dom.querySelector('#email').value;
        var password = this.dom.querySelector('#password').value;
        const candidate = {
          email: email,
          password: password
        };
        candidate.rol = 'CLI';
        var requestDireccion = `${backend}/users/login`;
    ...
    logout = async () => {
        ...
        let request = new Request(`${backend}/users/logout`, {
            method: 'DELETE',
